...
- Display names. Generated implementation identifiers (`gpt-o3_cuda_af0f3d`) render on list surfaces as a readable base with the short id receding ("gpt-o3 / cuda af0f3d", `splitImplementationName` in lib/names.ts); canonical identifiers stay untouched on the implementation and run dossiers and in manifests. Workload summaries render axis tokens as words ("batch 7", `formatWorkloadSummary`), and meta lines drop the GPU vendor prefix (`shortHardware`); the stored values remain the identity.
