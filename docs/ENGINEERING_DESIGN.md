...
- Header diet. ContextHeader context is one plain sentence, not a chain of middot-joined jargon; meta carries one stat cluster and at most one action. Sort controls live with the table they order. Page-local views (records' Current/Recently broken/History, the feed's All/Following) render as a left-aligned tab row under the title, never as right-aligned title metadata.
- Control grammar. Filter chips state their dimension: a faint label naming what kind of filter it is (Hardware = scope, Source = trust, Type = record kind) and the current value in the body tone, so three different conceptual dimensions never render as identical chips. A filter the corpus cannot currently satisfy (e.g. Verified while nothing is independently verified) is hidden, not dead-rendered; the fact moves to the page's methodology line. Non-obvious sorts (Contested) carry a native `title` explanation — no icons.
- Display names. Generated implementation identifiers (`gpt-o3_cuda_af0f3d`) render on list surfaces as a readable base with the short id receding ("gpt-o3 / cuda af0f3d", `splitImplementationName` in lib/names.ts); the implementation and run dossier H1s use the same display with the canonical identifier echoed in muted mono on the context line, and manifests stay untouched. Workload summaries render axis tokens as words ("batch 7", `formatWorkloadSummary`), and meta lines drop the GPU vendor prefix (`shortHardware`); the stored values remain the identity.
- Vocabulary bridges (lib/search-synonyms.ts). A small curated table maps common phrasings ("matrix multiplication") to the corpus's canonical tokens (matmul, gemm, mm), applied as alternates in both the suggest ranking and the server operation scorer — never replacing what was typed, never guessing beyond uncontroversial renames.
- Footer link clusters use flex gaps, never text-space separators, so arrowed links and the JSON ApiLink keep even spacing.
- /models is one catalog over both corpora: kernel coverage and serving coverage side by side per row (exact slug match, counts never summed or ranked together per §8.16), display names data-provided only (the serving source's name; kernel-only tags stay canonical mono), with a client-side filter. The serving console's bound group is titled "Scenario limits" and its copy states that MLPerf bounds compare against declared scenario rules, not measured latency. GPU dossiers show a serving-evidence section from the same resolver, scoped by hardware.
...
- **Module splits (§0.4 file-size hygiene):** every non-generated file over 1000 lines at the start of the pass was split 1:1 behind its existing import path (`publication.ts` crossed to ~1010 lines with the trait-extraction block and is the next split candidate) — `server/catalog/reads.ts` into `run-rows`/`cohorts`/`search-reads`/`operation-reads`/`implementation-reads`/`run-page-reads`/`compare-reads`/`home-reads` (reads.ts stays as the barrel the seam imports), `server/api/schemas.ts` into `schemas/{results,serving,dossiers,listings}.ts`, `lib/catalog-models.ts` into `lib/models/{rows,pages,listings,precedents}.ts`, and `data/fixtures/catalog.ts` into `data`/`search`/`dossiers`/`precedents` beside the barrel. Zero behavior change; importers unchanged.
